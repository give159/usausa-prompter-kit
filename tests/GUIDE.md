...
`npm test` まで、10ステップ。対象は「うさうさプロンプター v1.7C PRO」。掲載コマンド・出力はすべて実機確認済み（Node.js v22 / jsdom 29）。

## この記事のゴール

配布した `うさうさプロンプター_テスト一式_v1.7C.zip` を使い、手元で全21本のテストを緑にするまでを、コピペで追える10ステップにしました。読了目安10分、実作業10分です。
...
| レベル2 スモーク | jsdomでHTMLを丸ごと起動して検査（うち1本は`window.fetch`を監視し、通常版で通信そのものが起きないことを確認） | 6本 |
...
```
copy usausa-prompter-v1_7C-PRO.html usausa_tests\tool.html
```
...
`test/` 配下の `*.test.js` が自動で集まり、21本が一気に走ります（約3秒）。最後の3行だけ見ればOK。
...
これは書籍レビュー章の「AI無効化」を、机上ではなく実際に起動して裏づけるテストです。

ただしこの1本は**画面の案内文**しか見ていません。「案内は正しく表示されるが、裏では実際にAPIへ通信しようとしている」状態でも、このテストは緑になってしまいます。そこで、`window.fetch`を差し替えて監視し、送信ボタンを押しても実際に呼ばれていないことまで確認するテストを追加しています。

```js
test("通常版(ローカル)の安全確認: 送信ボタンを押しても fetch そのものが呼ばれない", async () => {
  let fetchCalled = false;
  const originalFetch = dom.window.fetch;
  dom.window.fetch = (...args) => { fetchCalled = true; return Promise.reject(new Error("test-stub")); };
...
「表示が正しい」ことと「裏の通信が起きていない」ことは別の確認です。ほかに起動確認・バージョン表示・localStorage保存（v:12／cpm:300／size:48）を検査します。
...
- HTMLを1箇所でも直したら `npm test`（21本で約3秒）
- バージョン刻印（PRO化）は fail 0 のときだけ。緑がPROの証
- 新機能は「観点→テスト→実装」の順。テストが仕様書代わりになる
- 別バージョン検査時は、smokeの「1.7C PRO」表示テストを新版数に書き換える

## 3行まとめ

- 解凍→`tool.html`配置→`npm install`→`npm test` の4手で全21本が緑になる

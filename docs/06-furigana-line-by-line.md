...
対象: うさうさプロンプター v1.7B PRO（全2,204行）。掲載コードはすべて実物そのままで、行番号も一致しています。
...
## 例4：色の読みやすさを判定 `contrastLabel`（996〜1002行目）
...
## 例7：保存のしくみ `lsSet` と `saveNow`（1314〜1322行目）
...
## 例8：安全の関門 `isArtifactEnv`（1969〜1977行目）
...
**読みどころ**：**「分からなければ安全側（無効）に倒す」**という設計になっています。3か所で false を返す道があり、true になるのは最後の1か所だけ。これが「通常版では通信しない」という本書の説明の、コード上の裏づけです。

---

## 例9：連打を防ぐ見張り番 `sendAI`（1936〜1938行目）

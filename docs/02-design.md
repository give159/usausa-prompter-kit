...
| notes | array | メモ（kind: todo／director／study） |
| highlights | object | 行ハイライト情報（行番号→色・下線） |
...
2. ZIP解析（1864〜1889行）: End of Central Directory Record（シグネチャ 0x06054b50）を末尾から探索し、各エントリのローカルヘッダからデータを取得
...
- `isArtifactEnv()`（1985行〜）が実行環境を判定。localhost／127.0.0.1／file:// は通常版。それ以外は、ホスト名が claude.site／claudeusercontent／anthropic を含む場合のみアーティファクト版と判定
...
- 会話は直近12件を送信し、各メッセージ本文は6,000字でクリップ。台本の整形・要約は4,000字、あいさつ・読み方のコツ用途は800字でクリップ
